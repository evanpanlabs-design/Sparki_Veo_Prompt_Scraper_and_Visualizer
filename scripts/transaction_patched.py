...
# Old format: 'ondemand.s': 'hash' (already broken)
ON_DEMAND_FILE_REGEX = re.compile(
    r"""['|\"]{1}ondemand\.s['|\"]{1}:\s*['|\"]{1}([\w]*)['|\"]{1}""", flags=(re.VERBOSE | re.MULTILINE))
# New webpack5 format: chunk_id:"ondemand.s" (e.g., 59924:"ondemand.s")
CHUNK_ID_REGEX = re.compile(r'(\d+):"ondemand\.s"')
# Hash lookup: chunk_id:"hash" pattern (e.g., 59924:"afb9ab7")
CHUNK_HASH_REGEX = re.compile(r'(\d+):"([a-f0-9]+)"')
...
class ClientTransaction:
    ...
    async def get_indices(self, home_page_response, session, headers):
        ...
        if on_demand_file:
            ...
            # New webpack5 format: chunk_id:"ondemand.s" → find hash in same response
            chunk_match = CHUNK_ID_REGEX.search(response_str)
            if chunk_match:
                chunk_id = chunk_match.group(1)
                # Look for the hash map entry: chunk_id:"hash"
                # e.g., 59924:"afb9ab7" somewhere else in the page
                hash_pattern = re.compile(rf'{chunk_id}:"([a-f0-9]+)"')
                hash_match = hash_pattern.search(response_str)
                if hash_match:
                    file_hash = hash_match.group(1)
                    # Filter out non-hash values like "ondemand.s" itself
                    if file_hash != 'ondemand.s' and len(file_hash) >= 6:
